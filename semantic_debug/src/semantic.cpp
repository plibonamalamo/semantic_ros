...
graph_node* graph_node_find_label(graph* g,const char* label){
	outf<< "\tCerco label " << label << std::endl;
	linked_list_iterator* it = linked_list_iterator_new(g->nodes);
	while(it!=NULL){
		outf<< "\tWHILE" << std::endl;
		...
		outf<< "CICLO: " << n_label << std::endl;
		if(strcmp(n_label,label)==0){
			outf<< "Trovato" << std::endl;
			return n;
		}
		outf<< "\tPasso " << n_prop->label << std::endl;
...
void poseCallback(const nav_msgs::Odometry::ConstPtr& msg) {
	...
	outf << "Trovo il piu vicino cont:" << cont << std::endl;
...
void speechCallback(const std_msgs::String::ConstPtr& msg){
	outf<< "ARRIVATO: " << msg->data << std::endl;
	std::string inizio;
	std::string fine;
	fine = prendi_fine(msg->data);
	inizio = prendi_inizio(msg->data);
	outf<< "INIZIO/FINE: " << inizio << "/" << fine << std::endl;
	if(inizio == "siamo in"){
		outf<< "siamo in " <<  std::endl;
		graph_node* nodo = graph_nearest_node(g,_X,_Y);
		outf<< "Trovato più vicino " <<  std::endl;
		
		char* label_c = str_to_char(fine);
		outf << "buffer contains: " << label_c << '\n';
		
		graph_node_set_label(nodo,label_c);
		outf<< "Settata label: " << fine << std::endl;
	}
	if(inizio == "vai in"){
		outf<< "Vai in" << std::endl;
		char* fine_c = str_to_char(fine);
		outf<< "Cerco " << fine_c << std::endl;
		graph_node* source = graph_nearest_node(g,_X,_Y);
		outf<< "Triovato source " << graph_node_get_label(source) << " " << graph_node_get_X(source) << std::endl;
		graph_node* dest = graph_node_find_label(g,fine_c);
		outf<< "Trovato dest " << graph_node_get_label(dest) << " " << graph_node_get_X(dest) << std::endl;
		linked_list* list = linked_list_new();
		outf<< "Creata List" << std::endl;
		graph_DFS(source,dest,list);
		flag_DFS = 0;
		outf<< "DFS <3" << std::endl;
		linked_list_iterator* it = linked_list_iterator_new(list);
		while(it!=NULL){
			outf<< "Ciclo nodi" << std::endl;
			graph_node* n = (graph_node*)linked_list_iterator_getvalue(it);
			float nx = graph_node_get_X(n);
			float ny = graph_node_get_Y(n);
			const char* n_lab = graph_node_get_label(n);
			outf<< "Nodo: " << n_lab << "X: " << nx << "Y: " << ny << std::endl;
			go_to(nx,ny);
			outf<< "Andato" << std::endl;
			it = linked_list_iterator_next(it);
		}
	}
}
